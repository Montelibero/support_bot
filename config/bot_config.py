...
class BotConfig:
    ...
    single_bot_token: Optional[str] = field(
        default_factory=lambda: env.str("SINGLE_BOT_TOKEN", None)
    )
    ...
    SINGLE_SECURITY_POLICY: str = field(
        default_factory=lambda: env.str("SINGLE_SECURITY_POLICY", "default")
    )
    SINGLE_MASTER_CHAT: int = field(
        default_factory=lambda: env.int("SINGLE_MASTER_CHAT", 0)
    )  # 0 means use ADMIN_ID
    SINGLE_USE_AUTO_REPLY: bool = field(
        default_factory=lambda: env.bool("SINGLE_USE_AUTO_REPLY", False)
    )
    SINGLE_AUTO_REPLY: str = field(
        default_factory=lambda: env.str(
            "SINGLE_AUTO_REPLY",
            "Message automatically forwarded to support. Please wait for a response.",
        )
    ...
    SQLITE_FILE_NAME: str = field(
        default_factory=lambda: os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "data", "support.db"
        )
    )
    ...
    def load_from_db(self) -> None:
        ...
        try:
            ...
            if not os.path.exists(self.SQLITE_FILE_NAME):
                logger.warning(
                    f"Database file not found: {self.SQLITE_FILE_NAME}. Skipping load."
                )
                return
            ...
            try:
                ...
                for row in rows:
                    ...
                    bot_dict = {
                        ...
                        if isinstance(row["local_names"], str)
                ...
                logger.warning(
                    f"Could not load settings from DB (table might be missing): {e}"
                )
            finally:
                conn.close()
        ...
        try:
            ...
            async with session_maker() as session:
                ...
                if bot_db:
                    ...
                    logger.warning(
                        f"Attempt to delete non-existent bot in DB with ID: {bot_id}"
                    )
            ...
            if bot_id_str in self.json_config:
                ...
                logger.warning(
                    f"Attempt to delete non-existent bot in cache with ID: {bot_id}"
                )
