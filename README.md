...
### Стоп-слова и блокировка контента
...
Опция `Игнорировать CJK-иероглифы` включена по умолчанию. Пока поддержка не
ответила пользователю, сообщения с CJK-иероглифами молча отбрасываются: они не
пересылаются в мастер-чат и не запускают автоответ. После первого ответа
поддержки фильтр для этого пользователя не применяется. Опцию можно отключить
отдельно для каждого support-бота.
