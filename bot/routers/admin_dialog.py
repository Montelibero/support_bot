...
async def get_bots(event_from_user: User, dialog_manager: DialogManager, **kwargs):
    ...
    bots = []
    user_bots = [
        item for item in config.get_bot_settings() if item.owner == event_from_user.id
    ]
...
async def mh_get_token(
    ...
    try:
        ...
        await message.answer(
            f"Бот @{bot_info.username} успешно добавлен. Переходим к настройкам."
        )
    ...
    if len(chat_data) >= 1:
        try:
            ...
            if not str(chat_id).startswith("-100") and chat_id > 0:
                await message.answer(
                    "ID чата должен начинаться с -100 для групповых чатов. Пожалуйста, проверьте ID."
                )
                return
            ...
            await message.answer(
                f"Чат успешно обновлен. ID чата: {chat_id}, \n"
            ...
            await message.answer(
                "Неверный формат. Пожалуйста, введите ID чата (и опционально ID топика) числами."
            )
    else:
        await message.answer("Пожалуйста, введите хотя бы ID чата.")


async def mh_change_start_message(
    message: Message, widget: MessageInput, dialog_manager: DialogManager
) -> None:
    state = dialog_manager.middleware_data["state"]
    data = await state.get_data()
...
async def mh_change_security_policy(
    message: Message, widget: MessageInput, dialog_manager: DialogManager
) -> None:
    state = dialog_manager.middleware_data["state"]
    data = await state.get_data()
...
async def mh_change_auto_reply(
    message: Message, widget: MessageInput, dialog_manager: DialogManager
) -> None:
    state = dialog_manager.middleware_data["state"]
    data = await state.get_data()
...
async def mh_change_owner(
    ...
    try:
        ...
        await message.bot.send_message(
            new_owner_id, f"Вам передали бота @{bot_setting.username}"
        )
        await message.bot.send_message(
            old_owner,
...
async def info_getter(dialog_manager: DialogManager, state: FSMContext, **kwargs):
    ...
    start_message_mini = (
        bot_setting.start_message[:100] + "..."
        if len(bot_setting.start_message) > 100
        else bot_setting.start_message
    )
    security_policy_mini = (
        bot_setting.security_policy[:100] + "..."
        if len(bot_setting.security_policy) > 100
        else bot_setting.security_policy
    )
    auto_reply_mini = (
        bot_setting.auto_reply[:100] + "..."
                    ...
                    try:
                        if bot_setting.master_chat is None:
                            await callback.answer(
                                "Чат не настроен. Укажите ID чата.", show_alert=True
                            )
                            return
                        await temp_bot.send_message(
                            chat_id=bot_setting.master_chat,
                            text="Проверка настроек чата",
                            message_thread_id=bot_setting.master_thread,
                        )
                        # Устанавливаем вебхук
                        webhook_url = config.other_bots_url.format(
                        ...
                        await callback.answer(
                            f"Ошибка доступа к чату: {str(e)}", show_alert=True
                        )
            except Exception as e:
                logger.exception(f"Activate bot failed — bot_id={bot_id}: {e}")
                await callback.answer(
                    f"Ошибка в настройках бота: {str(e)}", show_alert=True
                )
        else:
            try:
                async with make_bot(bot_setting.token) as temp_bot:
                ...
                await callback.answer(
                    f"Ошибка при деактивации бота: {str(e)}", show_alert=True
                )
...
window_bot_config = Window(
    Format(
        text="<b>Настройки бота: </b> @{username}\n\n"
        "{chat_info}\n\n"
    ...
    SwitchTo(Const("Вернуться к ботам"), state=AdminBotStates.main, id="to_main"),
...
window_send_chat_id = Window(
    Const(
        "Введите ID чата (обычно начинается с -100) и, если нужно, ID топика через пробел:"
    ),
