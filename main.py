...
def main():
    ...
    if os.environ.get("ENVIRONMENT") == "production":
        ...
        web.run_app(
            app, host=bot_config.WEB_SERVER_HOST, port=bot_config.WEB_SERVER_PORT
        )
    else:
        ...
        # Определяем типы обновлений, которые используются в обработчиках
        logger.info(
            f"Used update types for main_dispatcher: {main_dispatcher.resolve_used_update_types()}"
        )

        # Запускаем поллинг с оптимизированными типами обновлений
        asyncio.run(
            main_dispatcher.start_polling(
                bot, allowed_updates=main_dispatcher.resolve_used_update_types()
            )
        )
