...
async def test_ticket_flow(mock_server, repo):
    ...
    req_user = next(
        (
            ...
            for r in mock_server
            if r["method"] == "sendMessage"
            and str(r["data"]["chat_id"]) == str(USER_CHAT_ID)
            and "received" in r["data"]["text"]
        ),
        None,
    )
    ...
    update_admin_name = types.Update(
        ...
        message=types.Message(
            ...
            from_user=types.User(
                id=ADMIN_ID, is_bot=False, first_name="Admin", username="admin_tg"
            ),
    ...
    update_admin_reply = types.Update(
        ...
        message=types.Message(
            ...
            reply_to_message=types.Message(
                ...
                chat=types.Chat(id=MASTER_CHAT_ID, type="supergroup"),
                from_user=types.User(
                    id=123456, is_bot=True, first_name="Test Bot", username="test_bot"
                ),  # Bot sent it
                text="Forwarded text...",
            ),
            text=REPLY_TEXT,
