...
async def test_reply_to_deleted_message(mock_fail_server, repo):
    ...
    update_reply = types.Update(
        ...
        message=types.Message(
            ...
            from_user=types.User(
                id=ADMIN_ID, is_bot=False, first_name="Admin", username="admin"
            ),
            text="Reply Content",
            reply_to_message=types.Message(
                ...
                chat=types.Chat(id=MASTER_CHAT_ID, type="supergroup"),
                from_user=types.User(
                    id=123456, is_bot=True, first_name="Bot", username="bot"
                ),
    ...
    reply_attempts = [
        r
        for r in requests
        if r["method"] == "sendMessage" and "reply_to_message_id" in r["data"]
    ]
    ...
    fallback_attempts = [
        ...
        and "reply_to_message_id" not in r["data"]
        and r["data"]["chat_id"] == str(USER_CHAT_ID)
    ]

    # Ideally, after the fix, this assertion should PASS. For now, we expect it to FAIL or be empty.
    if not fallback_attempts:
        pytest.fail(
            "Bot did not retry sending the message without reply_to_message_id after receiving 'message not found' error"
        )
