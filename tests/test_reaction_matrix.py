...
async def test_reaction_matrix(repo):
    ...
    await repo.save_message_ids(
        ...
        message_id=300,
        resend_id=400,
        chat_from_id=MASTER_ID,
        chat_for_id=USER_ID,
    )

    # 2.A: User reacts to 400 (Received Reply) -> Should exist on 300 (Admin Original)
    update_2a = types.Update(
        update_id=3,
        message_reaction=MessageReactionUpdated(
            chat=Chat(id=USER_ID, type="private"),
            message_id=400,
            user=User(id=USER_ID, is_bot=False, first_name="User"),
            new_reaction=[ReactionTypeEmoji(emoji="C")],
            old_reaction=[],
            date=datetime.datetime.fromtimestamp(123),
        ),
    )
    ...
    update_2b = types.Update(
        ...
        message_reaction=MessageReactionUpdated(
            ...
            message_id=300,
            user=User(id=888, is_bot=False, first_name="Admin"),
            new_reaction=[ReactionTypeEmoji(emoji="D")],
            old_reaction=[],
            date=datetime.datetime.fromtimestamp(123),
        ),
    )
