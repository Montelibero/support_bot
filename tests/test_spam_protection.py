...
async def test_spam_protection(mock_server, repo):
    ...
    update_link_1 = types.Update(
        ...
        message=types.Message(
            ...
            from_user=types.User(
                id=USER_ID, is_bot=False, first_name="Spammer", username="spam_bot"
            ),
            text=text_with_link,
            entities=[entity],
        ),
    ...
    req_block = next(
        (
            ...
            and str(r["data"]["chat_id"]) == str(USER_ID)
        ),
        None,
    )
    ...
    update_image_no_link = types.Update(
        ...
        message=types.Message(
            ...
            from_user=types.User(
                id=NEW_USER_ID, is_bot=False, first_name="New Guy", username="new_guy"
            ),
            photo=[
                types.PhotoSize(
                    file_id="FILE_ID_123",
    ...
    req_block_photo = next(
        (
            r
            for r in mock_server
            if r["method"] == "sendMessage"
            and str(r["data"]["chat_id"]) == str(NEW_USER_ID)
        ),
        None,
    )
    ...
    req_photo_allowed = next(
        (
            r
            for r in mock_server
            if r["method"] == "sendPhoto"
            and str(r["data"]["chat_id"]) == str(MASTER_CHAT_ID)
        ),
        None,
    )
